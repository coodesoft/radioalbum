const Tiempo_anim = 500;
const Tiempo_aparicion_texto = 500;
...
function loadSuscriptionForm(){
	if ($('#suscriptionForm').length == 0){
		var form = createModalPopup('Suscribirse a Radioalbum', 'suscriptionForm');
		$(form).modal('show').find('.modal-body').load($('.suscribe > a').attr('href'));
	}
}

$(window).load(function(){
	$('.textoNot').each(function(){
		if ($(this).attr('data-expandido')=='1'){
			ExpandeArti($(this).attr('id').substring(1));
		}
	});
